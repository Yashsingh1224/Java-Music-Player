...
public class MusicPlayerApp extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        // Load songs into SongRecommender
        try {
            songRecommender.loadSongs("C:\\Users\\mrsta\\Documents\\ex.csv"); // Path to your CSV file
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Failed to load songs into recommender.");
        }
    ...
    private void playNextSong() {
        if (!recommendedSongsQueue.isEmpty()) {
            ...
            String nextSongTitle = recommendedSongsQueue.remove(0); // Remove from the queue
            System.out.println("Playing next recommended song: " + nextSongTitle); // Debug

            try (Connection connection = connectToDatabase();
                 PreparedStatement ps = connection.prepareStatement("SELECT title, artist FROM songs WHERE title = ?")) {

                ps.setString(1, nextSongTitle);
                ResultSet rs = ps.executeQuery();

                if (rs.next()) {
                    String songInfo = rs.getString("title") + " - " + rs.getString("artist");
                    playSongFromDB(songInfo); // Play the recommended song
                    return; // Exit once the recommended song starts playing
                } else {
                    System.out.println("Next recommended song not found in database: " + nextSongTitle);
                }
        ...
        // Fallback to the current playlist if no recommendations
        if (currentPlaylist != null && !currentPlaylist.isEmpty()) {
            currentSongIndex = (currentSongIndex + 1) % currentPlaylist.size();
            String nextSong = currentPlaylist.get(currentSongIndex);
            System.out.println("Playing next song from playlist: " + nextSong); // Debug
            playSongFromDB(nextSong);
        } else {
            System.out.println("No songs available in playlist or recommendations."); // Debug
        }
    ...
    private void dislikeSong() {
        System.out.println("Song Disliked!");
        ...
        try (Connection connection = connectToDatabase();
            ...
            if (rs.next()) {
                ...
                if (songFile.exists() && songFile.isFile()) {
                    ...
                    // Handle the case where the song file doesn't exist at the specified path
                    System.out.println("Error: Song file not found at the specified path: " + songFilePath);
                }
            }
    ...
    private void generateRecommendations(String songName) {
        ...
        for (Song song : recommendations) {
            ...
            recommendedSongsQueue.add(song.name); // Add recommended song names to the queue
        }
